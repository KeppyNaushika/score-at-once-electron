...
export async function saveTemplate(
  ...
  if (!examId || !currentUser || !selectedMasterImageId) {
    toast.error("試験ID、ユーザー情報、基準画像は必須です。")
  ...
  try {
    ...
    const savePromises = cropRegions.map(async (area) => {
      if (!area.examPageId) {
        throw new Error(
          `Crop region ${area.label || "Unnamed"} is missing examPageId.`
        )
      }

      const regionData = {
        examPageId: area.examPageId,
        type: area.type,
        x: area.x,
        y: area.y,
        width: area.width,
...
export function validateRegionData(region: CropRegionArea): {
  ...
  if (!region.examPageId) {
    errors.push("試験ページIDが設定されていません")
  }
