 ...
 */
export function useDragDropHandlers<TItem extends AnswerImageIdentity>({
  files,
  onFilesChange,
  getEnabledFiles,
  getDisabledFiles,
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (mode === "view") {
        ...
        if (!existingAnswers || !onUpdatePendingChanges) {
          setActiveFile(null)
          return
        }
        ...
        const changedFiles = diffFilesAgainstBaseline(newFiles, existingAnswers)
