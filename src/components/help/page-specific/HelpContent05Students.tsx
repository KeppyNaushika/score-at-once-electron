  ...
  return (
    <HelpDoc>
      ...
      <FocusSection title="① 画面の構成を知る">
        <p>
          画面の上部には2つのタブがあります。ふだんは「受験生徒一覧」タブで生徒の追加や受験状態の設定を行い、「学級の関連付け」タブで、生徒の学級と出席番号をどの学級から取るかを設定します。
        </p>
        <p>
      ...
      <FocusSection title="④ 並び順を整える">
        ...
        <p>
          手動で並べ替えた順番を元に戻したいときは、「並び順をリセット」ボタンを押します。学級の関連付けの順番（「再採番」にチェックの付いた学級）、そして学級内の出席番号の順に、自動で並び直されます。
        </p>
      </FocusSection>

      <FocusSection title="⑤ 学級を関連付ける">
        <p>
          「学級の関連付け」タブでは、生徒一覧や出力に載せる学級・出席番号をどの学級から取るかを管理します。生徒の追加そのものは「生徒を追加」ボタンから行います。「学級で追加」で生徒を追加した学級は、ここにも自動で並びます。
        </p>
        <p>
          「学級を追加」ボタンで学級を関連付けると、一覧に学級が並びます。各学級には「再採番」のチェックが1つあります。
        </p>
        <div className="border-l-4 border-blue-500 pl-3">
          <h4 className="text-sm font-bold text-blue-700">再採番</h4>
          <p className="text-sm text-gray-600">
            チェックの付いた学級の学級名と出席番号が、生徒一覧や出力に使われます。「並び順をリセット」もこの学級の順番と出席番号で並べ直します。生徒が複数の学級に入っているときは、一覧で上にある学級が優先されます。
          </p>
        </div>
        <p>
          学級の行はドラッグで並べ替えでき、ごみ箱のアイコンで関連付けを解除できます。関連付けを解除しても、その学級の生徒は受験生徒のまま残ります。
        </p>
        <Callout type="note" title="学級平均を出す学級は「9. 結果」で選びます">
          Excelの学級平均の行（教員集計）や、個人成績表の学級比較（生徒表示）にどの学級を出すかは、ここではなくステップ9「結果」で選びます。
        </Callout>
      </FocusSection>
