  ...
  return (
    <HelpDoc>
      ...
      <FocusSection title="① 表で領域の情報を設定する">
        ...
        <p>
          行のどこかをクリックすると、その行が選ばれます。もう一度クリックすると選択が外れます。種類・ラベル・配点の欄をクリックしたときは、その行が選ばれたまま入力に移ります。行の並び順は、上下にドラッグして入れ替えられます。
        </p>
      ...
      <FocusSection title="⑥ 採点する先生を割り当てる（採点担当）">
        <p>
          複数の先生で手分けして採点するときは、表の上にある「採点担当」のタブに切り替えます。縦に設問、横に先生が並んだ表が出るので、その先生に採点してほしい設問のマスにチェックを入れてください。担当を決めておくと、採点の画面にはその先生の担当する設問だけが並ぶので、迷わず進められます。
        </p>
        <ul className="ml-1 space-y-2">
          <li>
            マスをクリックすると右下に小さな四角（フィルハンドル）が出ます。これを下へドラッグすると、続いた設問へまとめてチェックを入れられます（表計算ソフトと同じ操作です）。
          </li>
          <li>
            担当を決められるのは、この試験を作った先生（所有者）だけです。ほかの先生には、だれが担当かが表示されます。
          </li>
          <li>
            チェックがひとつも無い設問は、全員が採点できます。割り当てを忘れても採点が止まることはありません。
          </li>
        </ul>
        <Callout type="note" title="ひとりで使うときは表示されません">
          この試験に参加している先生がひとりだけのときは、「採点担当」のタブそのものが表示されません。ほかの先生を招待すると現れます。採点結果が食い違ったときの調整は「8.
          採点確定」で行います。
        </Callout>
      </FocusSection>
      ...
      <FocusSection title="⑧ 合計配点を確認して次へ進む">
        <p>
          表の下には「合計配点」が表示されます。これは「設問解答」の配点をすべて足した点数です。試験全体の満点と合っているか、ここで確認しましょう。左側のページ一覧の下には、囲んだ領域の数が出ます。
        </p>
        <p>
          確認できたら、画面右上の「次へ:
