  ...
  return (
    <HelpDoc>
      ...
      <FocusSection title="⑤ あとから配置を直す">
        ...
        <p>
          変更を反映するときは、動かした答案ごとに採点をどうするかを確かめる画面が
          出ます。同じページの中で別の生徒へ動かした答案には
          <strong>「採点を移す」</strong>
          スイッチがあり、最初はオンです。オンのままなら、採点結果も答案と一緒に
          移ります。ふだんはオンのまま「反映」を押してください。
        </p>
        <Callout type="warning" title="採点が消える移動に注意">
          「採点を移す」をオフにした答案と、別のページのマス目へ動かした答案は、
          採点結果が消えるため採点し直しが必要です（別のページへ動かした答案は、
          採点を移せません）。そうした答案には「採点を破棄することを了解」の
          チェックが出るので、すべてにチェックを付けてから「反映」を押し、
          最後の確認で「破棄して反映」を押すと保存されます。
        </Callout>
        <p>
          答案は空いているマス目へもドラッグで動かせます。間違って入れた答案は、
          答案を右クリックして「答案画像を削除」を選ぶと削除できます。答案を
          新しく足すときは「新規追加」タブから取り込んでください。
        </p>
      </FocusSection>
