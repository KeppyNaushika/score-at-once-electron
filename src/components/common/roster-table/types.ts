...
export interface RosterTableAdapter {
  /**
   * この名簿が誰のものか（成績id・試験外成績資料id・試験id など）。
   *
   * **キャッシュのキーはこれで区切る。** 差し込み式なので「何の名簿か」は
   * アダプタを作った側にしか分からない。以前はコンポーネントの実体（`useId`）で
   * 区切っていたが、それは木の中の位置でしかなく、別の成績の同じ画面へ移ると
   * 同じキーになって前の名簿の行が出る（並べ替えれば、その行の id が新しい方の
   * 書き込みへ渡る）。
   */
  scopeId: string
  /** 名簿行を取得 */
