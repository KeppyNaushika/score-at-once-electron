...
export default function SubjectClassMatrix({
  ...
  const filteredStudents = students.filter((s) => {
    ...
    const homeroom = s.memberships.find(
      (m) => !m.endDate && !m.class.subject, // Homeroom classes don't have subjects
    )
    return homeroom?.class.grade === selectedGrade
  })

  // 教科別にクラスをグループ化
  const classesBySubject = filteredClasses.reduce(
    (acc, cls) => {
  ...
  // 生徒の現在の教科別所属を取得
  const getStudentClassForSubject = (
    student: StudentWithMemberships,
    subject: string,
  ) => {
    return student.memberships.find(
      (m) => !m.endDate && m.class.subject === subject,
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedSubject === "all" ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle>{selectedSubject}クラス配属表</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredStudents.map((student) => {
                    const currentMembership = getStudentClassForSubject(
                      student,
                      selectedSubject,
                    )
                    const subjectClasses =
                      classesBySubject[selectedSubject] || []
                      ...
                      <TableRow key={student.id}>
                        ...
                        <TableCell>
                          {currentMembership ? (
                            ...
                            >
                              {currentMembership.class.classCode ||
                                currentMembership.class.name}
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() =>
                                  onRemoveStudent(currentMembership.id)
                                }
                        ...
                        <TableCell>
                          <Select
                            ...
                            onValueChange={(classId) => {
                              if (
                                classId &&
                                classId !== currentMembership?.class.id
                              ) {
