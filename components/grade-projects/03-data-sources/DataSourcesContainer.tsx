...
import { useDataSources } from "@/hooks/grade-projects/useDataSources"
import type {
  AbsentMethod,
  GradeItemWithDetails,
...
export function DataSourcesContainer({
  ...
  const [students, setStudents] =
    useState<
      Awaited<
        ReturnType<typeof window.electronAPI.gradeProject.getStudents>
      >["students"]
    >()
  ...
  const allDataSources = project.gradeItems.flatMap((gi) => gi.dataSources)
  ...
  return (
    <div className="p-6">
      ...
      {/* GradeItem ごとのセクション */}
      {project.gradeItems.map((gradeItem) => (
      ...
      <div className="mt-8 flex justify-end">
        <Button asChild>
          <Link href={`/grade-projects/${gradeProjectId}/04-manual-scores`}>
            次へ: 外部成績入力
          </Link>
        </Button>
      </div>
