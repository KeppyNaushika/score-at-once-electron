...
export function MemberInviteDialog({
  ...
  const [members, setMembers] = useState<UserProjectWithUserAndInviter[]>([])
  ...
  const fetchMembers = useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await window.electronAPI.userProject.getMembers(projectId)
      setMembers(result)
    } catch (err) {
  ...
  const checkIsOwner = useCallback(async () => {
    ...
    try {
      const ownerStatus = await window.electronAPI.userProject.isOwner(
        currentUserId,
        projectId
      )
  ...
  const searchUsers = useCallback(async () => {
    ...
    setIsSearching(true)
    try {
      const results = await window.electronAPI.userProject.searchUsers(
        projectId,
        searchQuery.trim()
      )
  ...
  const handleInvite = async (userId: string) => {
    ...
    try {
      await window.electronAPI.userProject.invite({
        projectId,
        userId,
  ...
  const handleRemove = async (userId: string) => {
    ...
    try {
      await window.electronAPI.userProject.remove(
        projectId,
        userId,
        currentUserId
      )
