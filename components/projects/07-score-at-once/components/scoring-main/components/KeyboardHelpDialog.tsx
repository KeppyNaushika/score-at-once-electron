...
import { KEYBOARD_SHORTCUTS } from "@/components/projects/07-score-at-once/components/scoring-main/constants/keyboard-shortcuts"
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2 text-sm">
              {Object.entries(KEYBOARD_SHORTCUTS.SCORING).map(([key, shortcut]) => (
                <div key={key} className="flex justify-between">
                  <span>{shortcut.label}</span>
                  <code className="rounded bg-gray-100 px-2 py-1">{shortcut.key}</code>
                </div>
              ))}
            </div>
          </div>
              ...
              {Object.entries(KEYBOARD_SHORTCUTS.NAVIGATION).map(([key, shortcut]) => (
                <div key={key} className="flex justify-between">
                  ...
                  <code className="rounded bg-gray-100 px-2 py-1">
                    {shortcut.key === "採点キー" ? `${modifierKeyLabel}+採点キー` : shortcut.key}
                  </code>
                </div>
              ))}
            </div>
