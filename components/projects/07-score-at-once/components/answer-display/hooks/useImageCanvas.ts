...
import type { StudentAnswer, QuestionRegion, DrawingElement } from "../types/answer-display-types"
  ...
  const drawCanvas = useCallback(
    (images: HTMLImageElement[]) => {
      ...
      console.log("=== Canvas描画処理 ===", {
        ...
        imagesSources: images.map(img => img.src?.substring(img.src.lastIndexOf('/') + 1))
      })
        ...
        if (questionPageIndex >= 0 && questionPageIndex < images.length) {
          ...
          if (img) {
            ...
            for (let i = 0; i < questionPageIndex; i++) {
              pageOffsetY += images[i].naturalHeight * zoom + (images.length > 1 ? spacing : 0)
            }
          ...
          drawingElements.forEach((element) => {
            ...
            switch (element.type) {
              case "text":
                ...
                const actualFontSize = Math.max(12, (element.fontSize || 16) * scaleFactor)
        ...
        if (isSelected) {
            ...
            switch (currentDrawing.type) {
              case "line":
                if (
                  ...
                  const currentEndX = currentDrawing.endX * displayWidth + offsetX
                  const currentEndY = currentDrawing.endY * displayHeight + offsetY
          ...
          case "text":
            if (
              ...
              // テキストボックス作成中のプレビュー表示
              const currentWidth = currentDrawing.textBoxWidth * displayWidth
              const currentHeight = currentDrawing.textBoxHeight * displayHeight
              ctx.strokeStyle = currentDrawing.color || strokeColor
              ctx.lineWidth = 1
              ctx.setLineDash([5, 5])
              ctx.strokeRect(currentX, currentY, currentWidth, currentHeight)
  ...
  useEffect(() => {
    const loadAnswerImages = async () => {
      ...
      if (!answerSheet?.studentId) {
        console.warn("AnswerDisplayViewer: No student ID provided")
        return
      }
      ...
      if (showMultiplePages && allAnswerSheets) {
        ...
        console.log("全答案シート:", allAnswerSheets.map(sheet => ({
          id: sheet.id,
        ...
        console.log("同一生徒の答案シート:", studentAnswerSheets.map(sheet => ({
          id: sheet.id,
          pageNumber: sheet.pageNumber,
          imagePath: sheet.imagePath
        })))
        ...
        // 単一ページ表示：現在の答案のみ
        imagesToLoad = [{ path: answerSheet.imagePath, pageNumber: answerSheet.pageNumber }]
      ...
      const loadPromises = imagesToLoad.map(async (imageInfo) => {
        return new Promise<HTMLImageElement>((resolve, reject) => {
          ...
          img.onerror = (error) => {
            console.error(`Failed to load image for page ${imageInfo.pageNumber}:`, error)
            reject(error)
          }

          // ファイル存在確認して読み込み
          window.electronAPI.checkFileExists(imageInfo.path)
            .then(result => {
