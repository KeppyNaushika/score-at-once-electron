...
export function useAnswerDisplayEvents(props: UseAnswerDisplayEventsProps) {
  const {
    canvasRef,
    containerRef,
    imageRef,
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        const zoomDelta = e.deltaY > 0 ? ZOOM_SETTINGS.wheelDelta : ZOOM_SETTINGS.zoomInDelta
        const newZoom = Math.min(Math.max(zoom * zoomDelta, ZOOM_SETTINGS.min), ZOOM_SETTINGS.max)
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const imageCoords = screenToImageCoords(x, y, displayWidth, displayHeight, offsetX, offsetY)
        ...
        for (let i = drawingElements.length - 1; i >= 0; i--) {
          ...
          if (hitTestElement(element, imageCoords.x, imageCoords.y)) {
            ...
            if (element.type === "line") {
              ...
              const editMode = getRectangleEditMode(element, imageCoords.x, imageCoords.y)
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (isDraggingElement && selectedElementId && currentTool === "select") {
        const element = drawingElements.find(el => el.id === selectedElementId)
        if (element) {
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (isDrawing && currentDrawing) {
      if (currentDrawing.type === "text" && isCreatingTextBox) {
        ...
        // テキスト入力モーダルを表示
        if (canvasRef.current && currentDrawing.x !== undefined && currentDrawing.y !== undefined) {
          const img = imageRef.current
          if (img) {
            const displayWidth = img.naturalWidth * zoom
            const displayHeight = img.naturalHeight * zoom
            const offsetX = (canvasRef.current.width - displayWidth) / 2 - position.x
            const offsetY = (canvasRef.current.height - displayHeight) / 2 - position.y
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.key === "Delete" || e.key === "Backspace") && selectedElementId && !showTextInput) {
        e.preventDefault()
  ...
  }, [selectedElementId, showTextInput, setIsShiftPressed, removeDrawingElement])
