...
import type { TextBox, AnchorDirection } from "../../../../../app/textbox-on-canvas-v4/types"
import {
...
interface UseTextboxV4IntegrationReturn {
  ...
  showV4Modal: boolean
  /** V4統合モーダルを開く */
  openV4Modal: (position: { x: number; y: number }, initialText?: string, elementId?: string) => void
  /** V4統合モーダルを閉じる */
  closeV4Modal: () => void
  /** 現在編集中のテキスト値 */
...
export function useTextboxV4Integration({
  ...
  // DrawingElementをTextBoxに変換
  const convertToTextBox = useCallback((element: DrawingElement): TextBox | null => {
  ...
  // TextBoxをDrawingElementに変換
  const convertToDrawingElement = useCallback((textBox: TextBox): DrawingElement => {
    const drawingElementData = textBoxToDrawingElement(textBox, { canvasWidth, canvasHeight })
