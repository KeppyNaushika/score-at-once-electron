 ...
 */
export function convertAnnotationToElement(annotation: DrawingAnnotation): DrawingElement {
  return {
    id: annotation.id,
...
export interface UseDrawingAnnotationsReturn {
  ...
  // バッチ操作
  syncElements: (elements: DrawingElement[], questionScoreId: string) => Promise<DrawingAnnotation[]>
...
export function useDrawingAnnotations(
  ...
  const updateElement = useCallback(async (
    ...
    try {
      ...
      const result = await window.electronAPI.drawing.update(element.id, updateData)
  ...
  const deleteElement = useCallback(async (elementId: string): Promise<boolean> => {
    ...
    try {
      ...
      if (result.success) {
        setAnnotations(prev => prev.filter(annotation => annotation.id !== elementId))
      ...
      if (result.success) {
        ...
        if (type) {
          setAnnotations(prev => prev.filter(annotation => 
            !(annotation.questionScoreId === questionScoreId && annotation.type === type)
          ))
        } else {
          setAnnotations(prev => prev.filter(annotation => 
