...
import {
  calculateOptimalFontSizeV3,
  renderMarkdownToCanvasV3,
  type TextDimensionsV3,
} from "../../utils/canvasTextRendererV3"
...
export function useTextRenderCache() {
  ...
  // テキストを事前レンダリング
  const preRenderText = useCallback(
    async (element: DrawingElement, boxWidth: number, boxHeight: number) => {
  ...
  const preRenderElements = useCallback(
    async (
