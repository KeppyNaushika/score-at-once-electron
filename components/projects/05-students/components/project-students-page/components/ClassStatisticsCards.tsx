...
import type { ProjectClassWithClass } from "@/types/electron.d"
...
export function ClassStatisticsCards({
  ...
  const administeredClasses = projectClasses.filter(
    (pc) => pc.administered
  ).length
  const statisticsClasses = projectClasses.filter((pc) => pc.statistics).length
