...
import type { UnifiedFile, UnifiedStudent } from "@/components/projects/05-answer-sheets/answer-sheet-management/types"
...
export function AnswerSheetGridManager({
  ...
  const toggleCellState = useCallback((studentId: string, pageNumber: number) => {
  ...
  const handleUpload = useCallback(() => {
    const uploadData: Array<{ file: UnifiedFile; studentId: string; pageNumber: number }> = []
  ...
  }, [files, fileStates, sortedStudents.length, studentStates.size, maxPages, pageStates.size])
  ...
  return (
    ...
    >
        ...
        <CardHeader className="pb-4">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
            <div className="flex items-center gap-1">
              ...
              <span>ファイル: {stats.enabledFiles}/{stats.totalFiles}</span>
            </div>
            <div className="flex items-center gap-1">
              <Users className="h-4 w-4" />
              <span>生徒: {stats.totalStudents - stats.disabledStudents}/{stats.totalStudents}</span>
            </div>
            <div className="flex items-center gap-1">
              <Badge variant="secondary">
                ページ: {stats.totalPages - stats.disabledPages}/{stats.totalPages}
              </Badge>
            </div>
          </div>
        </CardHeader>
      ...
      <DragOverlay dropAnimation={null}>
        {activeFile ? (
          <div className="transform scale-110 rotate-3 rounded-lg border-2 border-blue-400 bg-white p-4 shadow-2xl">
            <div className="flex items-center gap-3">
              <div className="h-8 w-8 rounded bg-blue-200 flex-shrink-0" />
