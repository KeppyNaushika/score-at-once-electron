...
export default function ExportWarningModal({
  ...
  const hasWarnings = warnings.noScoringData.length > 0 ||
          ...
          {warnings.noScoringData.length > 0 && (
            <Alert className="border-red-200 bg-red-50">
              ...
              <AlertDescription className="text-red-800">
                <div className="font-medium mb-2">次の設問答案の採点データがありません</div>
                <div className="space-y-1 text-sm">
                  {warnings.noScoringData.map((item, index) => (
                    <div key={index} className="pl-2">• {item}</div>
                  ))}
                </div>
              </AlertDescription>
            </Alert>
          ...
          {warnings.ungraded.length > 0 && (
            <Alert className="border-orange-200 bg-orange-50">
              ...
              <AlertDescription className="text-orange-800">
                <div className="font-medium mb-2">次の設問答案が未採点です</div>
                <div className="space-y-1 text-sm">
          ...
          {warnings.missingPartialScore.length > 0 && (
            <Alert className="border-yellow-200 bg-yellow-50">
              ...
              <AlertDescription className="text-yellow-800">
                <div className="font-medium mb-2">次の部分点が入力されていません</div>
                <div className="space-y-1 text-sm">
