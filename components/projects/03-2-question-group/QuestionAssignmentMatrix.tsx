...
import { QuestionGroupWithItems, LayoutRegionWithDetails } from "../../../types/electron"

interface QuestionAssignmentMatrixProps {
  questionGroups: QuestionGroupWithItems[]
  layoutRegions: LayoutRegionWithDetails[]
  onUpdateAssignments: (questionLayoutRegionId: string, questionGroupItemIds: string[]) => Promise<boolean>
}
...
export function QuestionAssignmentMatrix({
  ...
  const [originalAssignments, setOriginalAssignments] = useState<AssignmentState>({})
  ...
  useEffect(() => {
    const loadAssignments = async () => {
      ...
      for (const region of layoutRegions) {
        try {
          const result = await window.electronAPI.getAssignmentsByQuestionLayoutRegionId(region.id) as any
          if (result && result.success && result.assignments) {
            newAssignments[region.id] = new Set(
              result.assignments.map((assignment: any) => assignment.questionGroupItemId)
            )
          } else {
            newAssignments[region.id] = new Set()
          }
      ...
      setOriginalAssignments(JSON.parse(JSON.stringify(
        Object.fromEntries(
          Object.entries(newAssignments).map(([key, value]) => [key, Array.from(value)])
        )
      )))
      setLoading(false)
    }
  ...
  // チェックボックスの状態を変更
  const handleAssignmentChange = (questionId: string, itemId: string, checked: boolean) => {
    setAssignments(prev => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setOriginalAssignments(JSON.parse(JSON.stringify(
        Object.fromEntries(
          Object.entries(assignments).map(([key, value]) => [key, Array.from(value)])
        )
      )))
    } catch (error) {
  ...
  const handleReset = () => {
    ...
    for (const [questionId, itemIds] of Object.entries(originalAssignments)) {
      resetAssignments[questionId] = new Set(Array.isArray(itemIds) ? itemIds : [])
    }
  ...
  if (loading) {
    return (
      <div className="text-center py-8">
        ...
        <p className="mt-2 text-muted-foreground">関連付けデータを読み込み中...</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {hasChanges() && (
            <Badge variant="outline" className="text-xs">未保存の変更があります</Badge>
          )}
        </div>
        ...
        <CardContent>
          <ScrollArea className="h-96 w-full">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-40 sticky left-0 bg-background z-10">設問</TableHead>
                  {questionGroups.map(group => (
                    <TableHead key={group.id} className="text-center min-w-32">
                ...
                {layoutRegions.map(region => (
                  <TableRow key={region.id}>
                    <TableCell className="sticky left-0 bg-background z-10">
                      <div className="space-y-1">
                        <div className="font-medium">{region.questionNumber || region.label}</div>
                        ...
                        <div className="grid gap-2">
                          {group.items.map(item => (
                            <div key={item.id} className="flex justify-center">
                              <Checkbox
                                checked={assignments[region.id]?.has(item.id) || false}
                                onCheckedChange={(checked) => 
                                  handleAssignmentChange(region.id, item.id, checked as boolean)
                                }
                              />
                            </div>
                          ))}
      ...
      <div className="text-sm text-muted-foreground bg-muted/50 p-4 rounded-lg">
        ...
        <ul className="space-y-1 ml-4">
          <li>• 各設問に対して、関連付けたいグループ項目にチェックを入れてください</li>
