...
interface QuestionGroupManagementProps {
  ...
  onCreateQuestionGroupItem: (questionGroupId: string, name: string) => Promise<boolean>
...
export function QuestionGroupManagement({
  ...
  const [editingGroup, setEditingGroup] = useState<QuestionGroupWithItems | null>(null)
  ...
  const handleEditGroup = async () => {
    ...
    const success = await onUpdateQuestionGroup(editingGroup.id, editGroupName.trim())
    if (success) {
      ...
      <div className="space-y-4">
        ...
        {questionGroups.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>設問グループがありません</p>
            <p className="text-sm">「新しいグループ」ボタンから作成してください</p>
          </div>
        ) : (
            ...
            {questionGroups.map((group) => (
              <Card 
                ...
                className={`cursor-pointer transition-colors hover:bg-muted/50 ${
                  selectedQuestionGroupId === group.id ? 'ring-2 ring-primary' : ''
                }`}
