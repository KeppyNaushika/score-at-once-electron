...
export function useQuestionGroupPage(examId: string) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [exam, setExam] = useState<any>(null)
  const [subtotalGroups, setSubtotalGroups] = useState<
    SubtotalGroupWithItems[]
  >([])
  // API returns simplified structure without examSubtotalGroups
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const loadData = useCallback(async () => {
    try {
      console.log("📥 loadData called for examId:", examId)
      setLoading(true)
      setError(null)
      ...
      const [
        ...
        window.electronAPI.getSubtotalGroupsByExamId(examId),
      ...
      if (
        ...
        const activeGroups =
          activeSubtotalGroupsResponse.examSubtotalGroups?.map(
            (psg: ExamSubtotalGroupWithSubtotalGroup) => psg.subtotalGroup
          ) || []
  ...
  }, [examId, selectedSubtotalGroupId])
