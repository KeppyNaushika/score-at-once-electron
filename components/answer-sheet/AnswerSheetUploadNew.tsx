...
import { Upload, FileImage, Users } from "lucide-react"
...
export default function AnswerSheetUploadNew({
    ...
    async (file: File, password?: string, onProgress?: (current: number, total: number) => void): Promise<ConvertedFileTemp[]> => {
      try {
        if (file.type === "application/pdf") {
          const convertedImages = await convertPdfToImages(file, password, onProgress)
      ...
      try {
        ...
        for (const file of fileList) {
          ...
          try {
            ...
            const convertedFiles = await convertSingleFileToImages(file, password, (currentPage, totalPagesInFile) => {
        ...
        if (process.env.NODE_ENV === "development") {
          console.log(`🎯 自動配置開始: masterImageCount=${masterImageCount}, 生徒数=${unifiedStudents.length}, ファイル数=${allConvertedFiles.length}`)
        }

        const newFiles: UnifiedFile[] = allConvertedFiles.map((convertedFile, index) => {
          // ページ優先配置ロジック（table-dnd-kit-test準拠）
          const studentIndex = Math.floor(index / masterImageCount) % unifiedStudents.length
  ...
  const handleUpload = useCallback(async (uploadData: UploadData[]) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(`${uploadData.length}件のファイルをアップロードしました`)
        
        // アップロード完了後、該当ファイルを削除
        const uploadedIds = new Set(uploadData.map(d => d.name.split(' - ページ')[0] + '-page-' + d.pageNumber))
        setFiles(prev => prev.filter(f => !uploadedIds.has(f.id.split('-').slice(0, -1).join('-'))))
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="grid grid-cols-4 gap-4 text-center">
              ...
              <div>
                <div className="text-2xl font-bold text-green-600">{stats.placedFiles}</div>
                <div className="text-sm text-muted-foreground">配置済み</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-orange-600">{stats.unplacedFiles}</div>
