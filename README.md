...
8. **採点確定** - 設問ごとの担当割り当てと、採点者間の食い違いの裁定
9. **結果出力** - PDF・Excel出力、統計プレビュー、プログレス表示付き

### 成績評定（6段階）

基本設定（評定名・参照日付）は概要ページでその場で編集します。

1. **対象生徒** - 評定対象の生徒を選択
2. **データソース** - 採点済み試験・試験外成績資料の紐付け
3. **外部成績** - 必要に応じた手動入力
4. **成績境界** - パーセンテージ閾値による評定ランク設定
5. **結果表示** - 評定結果の確認・確定（凍結）
6. **エクスポート** - 結果出力
