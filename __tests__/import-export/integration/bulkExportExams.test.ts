 ...
 * 実際のSQLiteテスト用DBを使用し、複数試験の順次エクスポートを検証する
 */
...
describe("executeBulkExport", () => {
  ...
  // BE-1: 複数試験の一括エクスポートが成功する
  it("BE-1: 複数試験を順次エクスポートし全て成功する", async () => {
    // 2つの試験を作成
    const exam1 = await createFullTestExam(prisma, {
      examName: "数学テスト",
      pageCount: 1,
      cropRegionsPerPage: 1,
      studentCount: 1,
    })
    const exam2 = await createFullTestExam(prisma, {
      examName: "英語テスト",
      pageCount: 1,
      cropRegionsPerPage: 1,
      studentCount: 1,
    ...
    expect(result.results[1].examName).toBe("英語テスト")
  ...
  it("BE-2: 存在しない試験IDは失敗し残りは続行する", async () => {
    const exam1 = await createFullTestExam(prisma, {
      examName: "成功テスト",
      pageCount: 1,
      cropRegionsPerPage: 1,
    ...
    expect(result.results[0].examId).toBe("non-existent-id")
    expect(result.results[0].error).toContain("試験が見つかりません")

    // 2番目の試験は成功
    expect(result.results[1].success).toBe(true)
    expect(result.results[1].examName).toBe("成功テスト")
  ...
  it("BE-3: 全試験が失敗した場合はsuccess=falseになる", async () => {
    const outputDir = path.join(tmpDir, "output")
    fs.mkdirSync(outputDir, { recursive: true })
  ...
  // BE-5: 空の配列を渡した場合
  it("BE-5: 空の試験配列ではsuccess=falseで結果も空", async () => {
    const outputDir = path.join(tmpDir, "output")
    fs.mkdirSync(outputDir, { recursive: true })
  ...
  // BE-6: 出力ファイルパスが各試験で正しいディレクトリ内に作成される
