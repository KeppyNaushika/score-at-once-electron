...
describe("transformExamArchiveToLatest", () => {
  ...
  test("1.23.0 → 1.24.0: 採点マークの採点者が落ちる（持ち主は親の採点データ）", () => {
    const data = createCurrentArchiveData()
    data.manifest.version = "1.23.0"
    putLegacyRows(data.scoresData, {
      questionScores: [
        {
          id: "qs-1",
    ...
    const result = transformExamArchiveToLatest(data)
    const [annotation] = result.data.scoresData.drawingAnnotations

    expect(annotation).not.toHaveProperty("userId")
    // 親は残るので持ち主は辿れる
    expect(annotation.questionScoreId).toBe("qs-1")
    expect(result.data.scoresData.questionScores[0].userId).toBe("user-1")
    expect(result.warnings).toEqual([])
  })

  test("1.23.0 → 1.24.0: 親と食い違う採点者は件数が警告に出る", () => {
    const data = createCurrentArchiveData()
    data.manifest.version = "1.23.0"
    putLegacyRows(data.scoresData, {
      questionScores: [
    ...
    expect(result.data.scoresData.drawingAnnotations[0]).not.toHaveProperty(
      "userId"
    )
  ...
  test("形状フロア: 版数が新しくても採点マークに userId があれば 1.23.0 まで引き下げる", () => {
    const data = createCurrentArchiveData()
    putLegacyRows(data.scoresData, {
      drawingAnnotations: [
        {
    ...
    expect(detection.corrections).toHaveLength(1)
    expect(detection.corrections[0]).toContain("DrawingAnnotation.userId")
  })

  test("1.24.0 → 1.25.0: 確定から採用元が落ち、判定と得点はそのまま残る", () => {
    const data = createCurrentArchiveData()
    data.manifest.version = "1.24.0"
    putLegacyRows(data.scoresData, {
      scoreDecisions: [
        {
          id: "sd-1",
    ...
    const result = transformExamArchiveToLatest(data)
    const [scoreDecision] = result.data.scoresData.scoreDecisions ?? []

    expect(scoreDecision).not.toHaveProperty("sourceQuestionScoreId")
    expect(scoreDecision.verdict).toBe("partial")
    expect(scoreDecision.score).toBe("3")
    expect(scoreDecision.comment).toBe("裁定の理由")
    expect(scoreDecision.decidedByUserId).toBe("user-1")
    expect(result.warnings).toHaveLength(1)
    expect(result.warnings[0]).toContain("1件")
  })

  test("1.24.0 → 1.25.0: 採用元が null の確定は警告を出さない", () => {
    const data = createCurrentArchiveData()
    data.manifest.version = "1.24.0"
    putLegacyRows(data.scoresData, {
      scoreDecisions: [
    ...
    expect(result.data.scoresData.scoreDecisions?.[0]).not.toHaveProperty(
      "sourceQuestionScoreId"
    )
    expect(result.warnings).toEqual([])
  })

  test("形状フロア: 版数が新しくても確定に採用元があれば 1.24.0 まで引き下げる", () => {
    const data = createCurrentArchiveData()
    putLegacyRows(data.scoresData, {
      scoreDecisions: [
        {
    ...
    expect(detection.version).toBe("1.24.0")
    expect(detection.corrections).toHaveLength(1)
    expect(detection.corrections[0]).toContain(
      "ScoreDecision.sourceQuestionScoreId"
    )
  })

  test("1.25.0 → 1.26.0: 覚え書きの無い採点行は空文字で補われる", () => {
    const data = createCurrentArchiveData()
    ...
    const result = transformExamArchiveToLatest(data)
    const [questionScore] = result.data.scoresData.questionScores

    expect(questionScore.comment).toBe("")
    // 判定・部分点はそのまま
    expect(questionScore.status).toBe("partial")
    expect(questionScore.partialScore).toBe("3")
    // 旧版には覚え書きを書ける画面が無かった＝失われた覚え書きが存在しない
    expect(result.warnings).toEqual([])
  })

  test("1.25.0 → 1.26.0: 既に書かれている覚え書きは上書きしない", () => {
    const data = createCurrentArchiveData()
    data.manifest.version = "1.25.0"
    putLegacyRows(data.scoresData, {
      questionScores: [
    ...
    expect(result.data.scoresData.questionScores[0].comment).toBe(
      "誤字は減点しない方針で3点"
    )
  })

  test("形状フロア: 版数が新しくても採点行に覚え書きが無ければ 1.25.0 まで引き下げる", () => {
    const data = createCurrentArchiveData()
    putLegacyRows(data.scoresData, {
      questionScores: [
        {
    ...
    expect(detection.version).toBe("1.25.0")
    expect(detection.corrections).toHaveLength(1)
    expect(detection.corrections[0]).toContain("QuestionScore.comment")
  })

  test("確定セクションが無い旧アーカイブに空配列を足さない", () => {
    const data = createCurrentArchiveData()
    data.manifest.version = "1.24.0"
    delete data.scoresData.scoreDecisions

    const result = transformExamArchiveToLatest(data)

    expect(result.data.scoresData.scoreDecisions).toBeUndefined()
  })
