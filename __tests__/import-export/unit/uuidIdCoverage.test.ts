 ...
 *
 * **ただし、この収束が成り立つのは「敗者行に子がいない場合に限る」。** 2026-08-19 の実測
 * （`docs/sync-secondary-unique-hazard.md`）で分かったのは次の2つで、どちらも画面に何も
 * 出ない。(a) 敗者行はカスケードで子ごと消えるので、その子は**世界から失われる**。
 * (b) 負けた側ではなく**勝った側の端末**が、子のINSERTで外部キー違反を起こす。適用は相手
 * 1人ぶんがまとめて1トランザクションなので取り込みが丸ごと巻き戻り、読んだ位置も進まない
 * ——**その相手からの以後すべての変更が永久に届かなくなる**。該当するのは「セカンダリ
 * unique を持ち、かつ子を持つ」10モデルで、最重は `ExamStudent`（協調採点の中核）。
 * 直すのはライブラリ側（負けた行の子を勝った行へ付け替える）で、本テストの対象ではない。
 *
 * **それでも、この不変式（アプリ側で id を組み立てない）は変えない。** 上の実測が崩すのは
 * 「LWWが畳むから id を組み立てなくてよい」という根拠の一方だけで、方針にはもう一本、
 * 独立の根拠がある —— **id が同一性を持つと、削除した id が再作成で復活するなど、行の生死と
