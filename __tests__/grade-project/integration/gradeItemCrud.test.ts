...
// テスト用GradeProject作成ヘルパー（テスト用クライアントを直接使用）
const testPrisma = new PrismaClient({
  datasources: { db: { url: `file:${TEST_DB_PATH}` } },
  log: ["error"],
})

async function createTestGradeProject(name = "テスト成績PJ") {
  return testPrisma.gradeProject.create({
    data: { name },
  })
}
...
describe("GradeItem CRUD", () => {
  ...
  describe("createGradeItem", () => {
    it("GradeItemを作成できる", async () => {
      const gp = await createTestGradeProject()
      ...
      expect(result.gradeItem!.gradeProjectId).toBe(gp.id)
    })
  ...
  describe("getGradeItemsByProjectId", () => {
    it("プロジェクトのGradeItem一覧を取得できる", async () => {
      const gp = await createTestGradeProject()
      await createGradeItem({ gradeProjectId: gp.id, name: "項目A" })
      await createGradeItem({ gradeProjectId: gp.id, name: "項目B" })

      const result = await getGradeItemsByProjectId(gp.id)
  ...
  describe("reorderGradeItems", () => {
    it("並び順を変更できる", async () => {
      ...
      const items = await getGradeItemsByProjectId(gp.id)
...
describe("GradeBoundary CRUD", () => {
  ...
  it("getBoundarySetsByGradeProjectIdで全セットを取得できる", async () => {
    const gp = await createTestGradeProject()
    const giResult = await createGradeItem({
      gradeProjectId: gp.id,
      name: "項目",
    ...
    const result = await getBoundarySetsByGradeProjectId(gp.id)
