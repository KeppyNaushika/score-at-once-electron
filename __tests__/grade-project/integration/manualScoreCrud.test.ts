...
/** テスト用のGradeProject + GradeItem + DataSource + Students を作成 */
async function createTestData() {
  ...
  const gradeItemResult = await createGradeItem({
    gradeProjectId: gradeProject.id,
    name: "知識・技能",
  })
  ...
  return { gradeProject, gradeItem, dataSource, student1, student2 }
}
  ...
  describe("deleteBoundarySet", () => {
    it("境界セットを削除できる", async () => {
      ...
      const remaining = await testPrisma.gradeBoundarySet.findMany({
        where: { gradeProjectId: gradeProject.id },
      })
