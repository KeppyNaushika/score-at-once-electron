...
**保存は差分更新にした（§1・§3）。** 残るものは id ごと残し、消えたものだけ消す。
OMR設定と選択肢も作り直しをやめた。作成日時のリセット（§3）も同時に消える。
検証は `__tests__/answer-sheet-builder/saveDiffUpdate.test.ts`。

#### 段階8 の残り（未着手）

**1. 同期の前提を実測する（最優先）**

差分更新は「delete→recreate が同期で消失を生む」という #1126 §1 の前提の上に立って
いるが、**その前提を実測していない**。CLAUDE.md が禁じている進め方をしている。

`~/dev/sqlite-nas-sync/__tests__/sync.test.ts` に2端末のハーネスがある。ここへ一時
シナリオを足して、旧挙動で端末Bから解答用紙が消えたまま復活しないことを**再現する**。
再現しなければ、この修正は存在しない問題への対処だったことになる（差分更新自体は
作成日時と担当が保たれるので正しく、巻き戻す必要はないが、**#1126 §1 の根拠を
書き換える**）。
...
**2. `getCurrentUser` の暫定実装を消す（#1127 の積み残し）**

`electron-src/lib/prisma/user.ts:16` は `prisma.user.findFirst()` を返す暫定実装で、
`// TODO: Implement actual current user retrieval logic` が付いたまま。残る呼び出しは
2箇所で、どちらも **「誰かがログインしているか」の門番**として使っている。

| 場所                                      | 使い道                           |
| ----------------------------------------- | -------------------------------- |
| `02-template/hooks/useTemplateData.ts:71` | `useCropRegionSave` の保存ガード |
| `03-region-info/page.tsx:69`              | `autoSaveRegions` の保存ガード   |

どちらも `userId` を書き込みには使っていない。ところが `findFirst()` は**誰も
ログインしていなくても非 null を返す**ので、門番として機能していない。

AuthContext の `user` に置き換えれば2箇所とも消え、`getCurrentUser` と
`get-current-user` チャンネルと暫定実装を**丸ごと削除**できる。認証の再設計は要らない。
#1127 が「前提になる」と書いた部分はこれで片付く。

**3. 規約の穴を2つ塞ぐ**

| 穴                                                                         | 塞ぎ方                                                                    |
| -------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `queryKeyConventions.test.ts` は `setQueryData` の書き込み側を見ていない   | 同じキーへ書く値の型が `queryFn` の戻り値と一致するかを型検査器で照合する |
| §7 の残り3つ（チャンネル突き合わせ・Decimal 走査・値 import 走査）が未実装 | テストとして書く。いま全部0件なのでグリーンのまま入る                     |

**4. 動作確認を e2e として足す**

**このブランチはまだ一度もアプリを起動していない。** 今回直したものの大半は型でも
ユニットテストでも出ない。基盤は既にある（`npm run test:e2e`、
`__tests__/tests/electron/`、`helpers/launchApp.ts` が一時データディレクトリで
新規インストール状態を作る）。

足すべきシナリオ（いずれも「画面を移って戻る」ことでしか出ない）:

| #   | 何を確かめるか                                                      | 出所              |
| --- | ------------------------------------------------------------------- | ----------------- |
| 1   | 04 を開いてから 03 へ移ると、ページと採点領域が出て編集が保存される | 段階7 #6          |
| 2   | 08 で出力設定を変え、07 へ移って戻ると、変えた設定のまま            | 段階8 (1)         |
| 3   | 資料の概要 → 評価項目 → 概要 を往復しても落ちない                   | 段階7 #1          |
| 4   | 解答用紙で編集 → 概要 → 作成 と戻っても編集が残る                   | 段階8 レビュー #1 |
| 5   | 担当でない利用者は解答用紙を編集できない                            | #1127             |
| 6   | 未ログインで 07 を開くとログインへ戻る                              | 段階8 レビュー #9 |

シナリオ1〜3は `__tests__/tests/electron/queryCacheRegressions.spec.ts` に書いた。
4〜6 は未着手。

> **走らせるときの注意が2つある。**
>
> - `npx vitest run` の後は better-sqlite3 が node 向けにリビルドされている。
>   `node scripts/ensure-sqlite-abi.js electron` を挟む（globalSetup も面倒を見る）
> - **3000番に別のセッションの dev サーバーが居ると、そちらのコードに対して
>   e2e が走る**（`reuseExistingServer: true`）。走らせる前に 3000番が自分のものか
>   確かめること。2026-08-13 時点では他セッションが 3000番を握っていて（500 を返す
>   状態）、**このブランチでは e2e を一度も通せていない**
