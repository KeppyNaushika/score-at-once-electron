...
| **R7**   | 段階17 の直後（ASB）    | `main...HEAD` 全差分 | 15件。**未着手** |
...
残る。**指摘15件は [branch-review-findings.md](./branch-review-findings.md) にある**
（1件ずつ「直すのか・向きが正しいのか」を決める作業表）。
...
**着手の前に、R7・R8 の指摘を片付けること。** 手写し型を直す作業は広い範囲へ同じ変更を
複製するので、振る舞いの誤りを抱えたまま入ると、直す場所が増えるだけになる。
