...
| 打鍵中の**下書き**       | 1打鍵ごとに書く入力欄は `useEditingText` が要る（下記）               |

**1打鍵ごとに書く入力欄には必ず `useEditingText` を使う。** 書き込みは `scope` で
直列化できるが、**それより前に始まった取得は止まらない**。取り直しが打鍵の合間に
着地すると入力欄が戻り、戻った文字列に次の打鍵が足されて DB に壊れた値が入る。

**`useMutation` の戻り値を effect の依存配列へ入れない。** 毎レンダー新しい
オブジェクトになるうえ、`mutate` 自身が再レンダーを起こすので、入れた瞬間に
自走するループになる。effect から呼ぶなら `useEffectEvent` で包む。段階9 の
「キー配列を依存配列へ入れた」と同じ家族で、`react-hooks/exhaustive-deps` は
どちらも「足せ」としか言わない。
...
| 5   | 03 のラベル入力が取り直しに上書きされ DB に壊れた値     | `useEditingText` を新設し3箇所で共有        |
...
**R1 で新しく増えた道具**（段階11以降でも使う）:

- `src/hooks/useEditingText.ts` — 1打鍵ごとに書く入力欄の下書き
- `src/hooks/useWritePreference.ts` — 設定を書く唯一の口（符号化＋楽観更新）
