...
**DB に残っている「設定を持つ JSON」を無くす。** 試験の出力設定を6テーブルへ割った
段階12 と同じことを、利用者の設定と成績の出力設定に対して行う。

置き場所を移すだけだった段階10〜14 と違い、ここは**スキーマが変わる**。マイグレーションと
（成績アーカイブについては）変換器が要る。

#### なぜ段階19 より前か

段階19（手写し型の是正）は「対象が確定してからやる」段階で、その理由は ASB の RDB 化で
5件が消えることだった。**設定 JSON でも同じことが起きる。** `ScoringStatusColors` などは
「JSON の形の型」から「Prisma の行型」へ変わるので、先に割らないと、19 で直して検査を
入れた直後に対象が変わる。ASB（段階15〜17）とは独立なので、順序の制約は「19 より前」だけ。

#### きっかけ

R5 で見つけた「採点状態色が一切保存されない」は、保存の符号化（`serializePreference` が
文字列を JSON でくるむ）と中身の JSON が二重になったところで起きた。段を数える場所を
1つにして塞いだが、**行へ割れば数える段が無くなる**。あわせて `as` が隠していた
「保存に無い状態を読むと描画で落ちる」も、列があれば起こらない。

**R6 で理由がもう1つ増えた。** 楽観更新をやめたので、JSON をまるごと読み書きする設定は
**続けて2回変えると先の1回が消える**（取り直しが着地する前に、古い写しへ2回目を重ねて
書くため）。実測で挙がったのは、クリック採点の動作を続けて変える・側面パネルの節を
続けて畳む・採点状態の色を続けて2つ変える・成績の出力設定のチェックを続けて2つ入れる、
の4つ。**行へ割れば別々の行を書くので、競合そのものが無くなる。**

同じ形でも**文字を打つ欄だけは行へ割っても直らない**（打鍵のたびに書く以上、取り直しが
着地するまで表示が古い）。そちらは下書きを手元に持つ（`useEditingText`）。R6 で
成績の出力設定へ入れた。

#### 対象（DB の JSON は4箇所。割るのは2つ）

| 場所                               | 割った後                                        | アーカイブ          | 変換器                                     |
| ---------------------------------- | ----------------------------------------------- | ------------------- | ------------------------------------------ |
| `UserPreference` の3キー           | 利用者×採点状態 / 利用者×クリック数 / 利用者×節 | **入っていない**    | **不要**                                   |
| `GradeExportSettings.settingsJson` | 段階12 の試験側と同じ形の行                     | 成績（現行 1.14.0） | **要る**（1.15.0 ＋ `V1_14_0_to_V1_15_0`） |
| `ReturnSnapshot.scoresJson`        | —                                               | —                   | **対象外**                                 |
| `AuditLog.metadata`                | —                                               | —                   | **対象外**                                 |

対象外の2つは名指しで除く（判断基準は書かない）。`scoresJson` は**丸ごと比較して差分を
出すのが目的**なので、行へ割ると比較が高くなる。`metadata` は
[audit-log-redesign.md](./audit-log-redesign.md) が持つ別の枠。

`UserPreference` がアーカイブに入っていないことは実測で確かめた（`electron-src/lib/export`
と `import` の双方に参照が無い）。

#### 手順

1. **18-a `UserPreference`** — schema ＋ 手書き migration（JSON→行）＋ `src/queries/settings.ts`
   の口を割る ＋ 設定画面・07。アーカイブ非対象なのでここで閉じる
2. **18-b `GradeExportSettings`** — schema ＋ migration ＋ **成績アーカイブ 1.15.0 ＋ 変換器**
   ＋ collector / importer / extractor ＋ 成績 07-export

#### 分かっていること・気をつけること

- **既存の設定は移せる。** `better-sqlite3` で `json_extract` と `json_each` が使えることを
  実測した。移行SQLで JSON から行へ移すので、利用者が設定した配色は消えない
- **`migration.sql` は手書き**（Prisma7 の datasource に url が無く `migrate dev` を実行
  できない）。起動時に `migrationDeployer` が適用する
- **順序破壊に注意。** 後発の列が過去の migration に混入して全新規DBが起動不能になった
  実例がある。空DBへ全SQL昇順適用で検証する
- 移行前の保存値には**無い状態がある**（`double_mark` などは後から増えた）。行へ移すときは
  「無いものは入れない」で足りる（読む側は既定で埋める）

**完了条件**: 設定を持つ JSON 列が無くなる（残るのは名指しの2件）。
