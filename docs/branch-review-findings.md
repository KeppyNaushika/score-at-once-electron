...
**16件すべて対応済み**（2026-08-20）。本書は**済んだことの記録**である。同じく済んだ
段の記録は [completed-work.md](./completed-work.md)、これからのことは
...
| 件               | いまどうなっているか                                                                                                                                                                                                                                        |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **#12**          | 決定は「**書き出す前に**確認して続行を選ばせる」だったが「作ってから伝える」にした（**OWNER 了承・2026-08-20**）。**対応済**（`127e9496`）— 結果をモーダルで見せ、書き出し元ごとに欠けたファイル名を全部出す。試験1件・一括・`.asb` の3経路が同じ部品を通る |
| **#10 の余波**   | **対応済み**（2026-08-20）。一括タグ付けの選択を担当分に限り、外した件数を伝えるようにした                                                                                                                                                                  |
| **#13**          | 鍵を `scopeKeys.exam(examId)` の中へ入れる分は**やらない**（当初「見送り」と書いたが理由が違った）。次節を参照                                                                                                                                              |
| **#13 の残る窓** | **決着**（2026-08-20）。数え直して中止する形を**全ての削除へ広げる**（OWNER 決定）。[completed-work.md](./completed-work.md) 段階26                                                                                                                         |
...
## 12. 画像が欠けた書き出しを成功と報告する（中・確認済・**対応済**）
...
**`exportExamTo` も `exportExam` も読まずに捨てていた**。
...
### 決定 — 「作ってから伝える」（OWNER 了承・2026-08-20）

当初の決定は「**書き出す前に**確認して続行を選ばせる」だった。**変えた。** 欠落が分かるのは
アーカイブ生成の最中で、事前に知るには収集後の存在確認をもう一周する必要があり、しかも
同じ経路を**一括書き出し**が通るのでダイアログを置けない。

#7（1ページでも失敗したら中止）とは**性質が違う**。#7 は PDF が壊れた成果物になるが、#12 は
ZIP 自体は正しく、中身の一部が無いだけで取り込みは成功する。**中止ではなく、伝える**。

### 実施（2026-08-20・`127e9496`）

**結果をトーストではなくモーダルの中で見せる形にした。** 書き出し元ごとに件数を並べ、
開くと欠けたファイル名が全部出る。

- `BulkExportExamResult` に `missingFiles` を足し、`archiveHandlers` が積む
- `ExportModeModal` は結果まで持つようになったので `ExamArchiveExportModal` へ改名
  （re-export は残さず import 元を直接書き換え）
- **3経路が同じ部品・同じ文言を通る** — 試験1件・試験の一括・解答用紙（`.asb`）
- `.asb` は `missingFiles` を集めながら捨てていた（配列がどこへも行かない死んだ変数）ので、
  返り値に載せ、監査ログの `extra` にも残した

**当初の指摘に無かったものも一緒に直した。** 一括書き出しは失敗した試験を「N件失敗」と
数でしか伝えておらず、`error` を試験ごとに持っていながら画面が捨てていた。同じ
「伝えずに済ませている」形なので、結果のモーダルに並べた。

### 保留していた理由は解消した

「`missingFiles` に何が入るのか（`exportMode` によっては欠けているのが正常かもしれない）」を
確かめずにいたが、`archiveCreator.ts:209/222` が入れるのは **`existsSync` が false だった
ファイルだけ**で、モードによる除外は収集の段階で起きるため混ざらない。
