...
| 件             | 何を残したか                                                                                                                                                                                                                                                     |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **#12**        | 決定は「**書き出す前に**確認して続行を選ばせる」だったが「作ってから伝える」にした。欠落が分かるのはアーカイブ生成の最中で、事前に知るには収集後の存在確認をもう一周する必要があり、同じ経路を**一括書き出し**が通るのでダイアログを置けない。事前確認まで要るか |
| **#13**        | 鍵を `scopeKeys.exam(examId)` の中へ入れる分を見送った。この modal は `examId` を持たず、3階層 props を通す必要がある。しかも無効化は**他の教員が採点した場合には届かない**ので、入れた「開くたびの取り直し」が上位互換                                          |
| **#10 の余波** | 一括タグ付け（`addTagToAnswerSheetDefinitionsMutation`）は per-definition の `catch` で unique 違反を飛ばしている。担当の確認を通したので、**担当でない解答用紙も同じ catch で黙って飛ばされる**。利用者に伝わらない                                             |
...
| 13  | 中   | `DeleteConfirmationModal.tsx:65`        | 採点済みの答案を「採点なし」と告げて消せる           | 確認済 | 一部済              |
