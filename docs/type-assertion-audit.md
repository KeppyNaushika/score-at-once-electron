...
**`as` は 620 箇所あった**（初回計測。**2026-09-09 に同じ AST スクリプトで再計測して 549 箇所** ──
うち `__tests__` が 248、`src` + `electron-src` が 301。二重アサーション `as unknown as` は 69）。
...
「調査時」は 2026-08-02 の初回計測、「現況」は **2026-08-03 時点**の再計測。

> **群ごとの内訳は 2026-08-03 のまま。** 全体は 2026-09-09 に再計測して **602 → 549** へ
> 減っているが、その差分を群へ割り当て直してはいない。着手する前に §2 の方法で計測し直すこと。
...
- **`@@unique([examId, pageNumber])` で重複を防ぐ** — sync 構成では各端末がローカル DB へ書き、
  マージは行レベルなので、制約は各端末で満たされ衝突はマージ時に現れる。防げないうえ、
  `pageNumber` は序数なので unique にできない（端末Aの3ページ目と端末Bの3ページ目は別の行で、
  値が一致するのは偶然。畳むと別物が1つに潰れる。規約は「uuid 以外を unique にしない」）
...
| 順  | 対象                                                                                     | 件数 | 性質           | 次の一手                                                           |
| :-: | ---------------------------------------------------------------------------------------- | ---: | -------------- | ------------------------------------------------------------------ |
|  1  | B群のうち `electron-src/lib/prisma/asbDefinitionConverters.ts`（2026-09-09 時点で 33件） |   34 | 予防・中コスト | ASB 系 union を `defineStringUnion` の3点セットへ。UI 側13件も同時 |
...
| 2026-09-09 | 全体を再計測（602 → 549）。群ごとの内訳は 2026-08-03 のまま。`asbDefinitionConverters.ts` の所在を `electron-src/lib/prisma/` へ訂正 |
