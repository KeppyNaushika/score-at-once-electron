...
**段階1〜5 は完了**（2026-08-19）。**段階6（原稿用紙のテーブル化）は未着手**で、着手の計画は
[completed-work.md](./completed-work.md) に **段階24** として置いた（2つの数列が混ざるのを
避けるため、あちらの通し番号へ寄せてある）。設計は本書 §8.5。
...
### 7.6 段階6 — 原稿用紙をテーブルへ出し、枝問へも生やす（済 2026-08-21 ＝ **段階24**）

設計（スキーマ・移行の手順と罠・画面側の型・作業の並び・検査）は本書 §8.5。

段階1〜5 が「書き込みを実体ごとに割る」だったのに対し、これは**実体そのものを正す**。

実施の結果と、計画からの差:

1. `AsbManuscriptPaper` を作り、`AsbCharGuide` の親を小問から原稿用紙へ付け替え、
   `AsbSubQuestion` から7列を落とした（migration `20260821000000_move_asb_manuscript_paper_to_table`）
2. **小問の7列は `RedefineTables` ではなく `DROP COLUMN` で落とした。** 前段が足した列を
   `CREATE TABLE` の書き写しで取りこぼす事故（`AsbDefinition` の4列欠落）を、起こしようが
   なくするため。作り直しが要るのは `AsbCharGuide` だけ（列名と外部キーの両方が変わる）
3. **枝問の原稿用紙は、解答欄そのものをマス目の幅にする**（`ansW = gridW`）。小問がやって
   いる「必要幅から `layoutWidth` を逆算する」上乗せは入れていない — その逆算は
   `computeMajorHeight` が用紙幅を知っているから成立する話で、枝問側の高さを出す
   `computeSubHeight` には用紙幅が渡っていない。入れると詰め方が場所によって食い違う
4. 画面側の型は `AsbManuscriptPaperAttributes`（Prisma 由来。`guidePosition` だけ union を
   注入）と `ManuscriptPaper`（＝ include の木）になり、`AsbSubQuestionUpdate` の入れ子の
   例外が消えた
5. アーカイブ（`.asb`）は **1.4.0**。変換器 `V1_3_0_to_V1_4_0`
...
[completed-work.md](./completed-work.md) の段階20）で、id を決定論的にする案は採らない
（同文書 §6.3）。
...
3. `id` は **uuidv4 を SQL で作って入れる**（§決めた形。借用 id は禁止で、
   `uuidIdCoverage.test.ts` が見張っている）。式は
   `20260704000000_add_asb_char_guide` に先例がある
4. `AsbCharGuide` を作り直し、`subQuestionId` から引いた原稿用紙の id を
   `manuscriptPaperId` として写す
5. `AsbSubQuestion` から7列を落とす
...
| id      | 原稿用紙の id が uuidv4 で、親の id と一致しない（借用 id の禁止）           |
...
| 2026-08-21 | 段階6 を実施。§7.6 に結果と計画からの差を記録。§8.5 の移行手順・検査に残っていた「id は親の id を借りる」（撤回済み）を訂正 |
