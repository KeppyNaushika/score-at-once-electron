...
__tests__/
├── helpers/                         # テスト用ヘルパー
│   ├── testProjectBuilder.ts          # DBにプロジェクトデータ一式を作成
...
│   │   ├── collectProjectData.test.ts   # データ収集パイプライン
...
DBに完全なプロジェクトデータを一式作成するヘルパー。

| 関数                                     | 説明                                                                                                    |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `createFullTestProject(prisma, options)` | User, Project, Pages, CropRegions, Students, Class, QuestionScores 等を一括作成し、全エンティティを返す |
...
| `createTestImageFiles(baseDir, projectId, pageCount, studentNumbers)` | マスター画像・答案画像のディレクトリ構造を作成 |
...
| 関数                                                                            | 説明                                           |
| ------------------------------------------------------------------------------- | ---------------------------------------------- |
| `createTestArchive(collectedData, outputPath, projectId, projectName, options)` | CollectedDataからZIPアーカイブ（.score）を作成 |
...
| MV-2 | 必須フィールド欠落で失敗する                   | version, exportedAt, projectId, projectName, counts の各欠落でエラー |
...
| AC-3 | 日本語のプロジェクト名がそのまま保持される | "数学Ⅰ　期末考査　2025年度" が維持される |
...
| プロジェクトデータがデフォルト設定で生成される             | ArchiveProjectDataのデフォルト |
| ページ数とリージョン数を指定できる                         | カスタム設定                   |
| cropRegionのprojectPageIdが正しいページを参照する          | FK整合性                       |
...
### collectProjectData.test.ts（12テスト）

DBからプロジェクトデータを収集する `collectProjectData` 関数を検証。

| ID    | テスト名                                               | 検証内容                       |
| ----- | ------------------------------------------------------ | ------------------------------ |
| DC-1  | 全エンティティ型を含むプロジェクトのデータが収集される | 全データ種別の収集確認         |
| DC-2  | 現在ユーザーの採点データのみ収集される                 | ユーザー別フィルタリング       |
| DC-3  | 現在ユーザーのアノテーションのみ収集される             | アノテーションのフィルタ       |
| DC-4  | 存在しないプロジェクトIDでエラーが返る                 | 異常系：存在しないプロジェクト |
...
| DC-9  | v1.4.0データが収集される                               | ProjectMarkingFormat等の収集   |
...
インポート前の事前照合（生徒・学級・小計グループ・プロジェクトの一致判定）を検証。
...
| PM-7 | プロジェクトIDが一致する場合                | isIdMatch=true       |
| PM-8 | プロジェクトIDが不一致の場合                | isIdMatch=false      |
...
| II-4  | 同一プロジェクト再インポートでスコアがunchangedとなる | 再インポートの冪等性     |
...
| II-12 | v1.4.0: ProjectMarkingFormatが作成される              | 新規データ型の処理       |
| II-13 | v1.4.0: ProjectExportSettingsが作成される             | 新規データ型の処理       |
...
| II-18 | ProjectClassesが正しくマッピングされる                | 学級-プロジェクト関連    |
...
| 複数のFK参照が全て更新される                                       | ProjectSubtotalGroup + Subtotal |
...
| プロジェクトIDが一致しない場合、全て新規として扱い競合なし              | プロジェクト不一致       |
...
| E2E-2 | 既存プロジェクトへのマージインポート                        | 既存データとの統合     |
...
| ID   | テスト名                                                    | 検証内容           |
| ---- | ----------------------------------------------------------- | ------------------ |
| EC-1 | 空プロジェクト（生徒・スコアなし）がインポートできる        | 最小データの処理   |
...
| B3     | ProjectMarkingFormatがインポートで処理されることを確認            | パス |
| B3     | ProjectExportSettingsがインポートで処理されることを確認           | パス |
| B3     | CropRegionMarkingOverrideがインポートで処理されることを確認       | パス |
| B3     | Subject/SubjectSubtotalGroupがインポートで処理されることを確認    | パス |
| B3     | ProjectClassがインポートで処理されることを確認                    | パス |
...
| B7     | processProjectでexistingById検出時に警告が追加される              | todo |
| B8     | idMappings.projectで正しいIDを取得                                | パス |
...
| ID   | テスト名                                               | 検証内容                     |
| ---- | ------------------------------------------------------ | ---------------------------- |
| FO-1 | プロジェクト名がmanifestから表示される                 | プロジェクト名表示           |
...
| EX-4 | 成功時にプロジェクトを開くボタンが表示される            | 成功時のアクション |
| EX-5 | 失敗時にエラーメッセージが表示される                    | エラー表示         |
| EX-6 | プロジェクトを開くボタンでonCompleteとonCloseが呼ばれる | コールバック動作   |
