...
### renderer — 1画面1往復。ただし寿命の違うものは分ける（厳守）

**1つの画面（`page.tsx`）が同じデータを2通りの経路で取らない。** 過不足なく同じものが
要るなら共用してよいが、少しでも違うなら分ける。同じ行が2つのキーに載ると、どちらが
正かが決まらなくなる。

**その上で、寿命の違うものは同じ往復に載せない。** 「めったに変わらないもの」と
「1打鍵ごとに変わるもの」を1つのキーへ入れると、書くたびに変わらない側まで取り直す
ことになる。

**取り直しはキーより細かくできない。** クエリの取り直しは `queryFn` を呼び直すこと、
つまりその値を丸ごと作り直すことなので、「キャッシュの一部だけ取り直す」という操作は
存在しない（それをやるのが `setQueryData` ＝ 楽観更新で、この文書は既定で禁じている）。
したがって**キーの粒度が取り直しの下限**になる。細かく持っておけば狭くも広くも選べるが、
太いまま持つと広くしか選べない。

実例（段階70）。採点行を採点領域の木に子として載せていたので、1マス採点するたびに
**その試験の全採点行**を取り直していた ——**画面に出ているのは1設問ぶんなのに**。
木から外して設問ごとのキーへ割ると、取り直す範囲が**書いた設問1つぶん**になった
（**設問の数だけ小さくなる**）。同じ木を引いていた 02・03・04・06 は採点行を1行も
使っていなかったので、そちらは採点行が丸ごと落ちた。

`include` を1つ足す判断は、**その木を引いている全部の画面に配られる**。足す前に、
使わない画面が何を余分に受け取るかを数えること。
