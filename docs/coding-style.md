...
木をまるごと受け取って保存する経路（解答用紙・観点間制約など）で、**受け取った分を
全部書き直さない**。id で突き合わせて、
...
**理由は同期。** DB は NAS 越しに複数端末で共有され、収束は行ごとの LWW
（`updatedAt` が新しい方が勝つ）で決まる。全行を書き直すと触っていない行まで
`updatedAt` が「今」になるので、**2端末が別々の大問を編集しただけで、後から保存した
側の木が丸ごと勝ち、相手の編集が消える**。触った行だけを書けば、重ならない編集は
両方残り、失われるのは本当に同じ行を同時に触ったときだけになる。

「削除だけ差分、更新は全書き換え」は差分更新ではない。
