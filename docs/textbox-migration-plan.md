...
export function setupDrawingHandlers() {
  ...
  ipcMain.handle(
    ...
    async (_, questionScoreId, type) => {
      return await drawingService.getDrawingAnnotationsByQuestionScore(
        questionScoreId,
        type,
      )
    },
  ...
  ipcMain.handle(
    ...
    async (_, questionScoreId, type) => {
      return await drawingService.deleteDrawingAnnotationsByQuestionScore(
        questionScoreId,
        type,
      )
    },
...
export function useDrawingAnnotations(questionScoreId: string | null) {
  ...
  const loadAnnotations = useCallback(
    async (type?: DrawingType) => {
      if (!questionScoreId) return
      const data = await window.electronAPI.drawing.getByQuestionScore(
        questionScoreId,
        type,
      )
      setAnnotations(data)
  ...
  const createAnnotation = useCallback(
    async (annotationData: DrawingCreateData) => {
      const newAnnotation =
        await window.electronAPI.drawing.create(annotationData)
  ...
  const updateAnnotation = useCallback(
    async (id: string, updateData: DrawingUpdateData) => {
      const updatedAnnotation = await window.electronAPI.drawing.update(
        id,
        updateData,
      )
      setAnnotations((prev) =>
  ...
  const batchUpdate = useCallback(
    async (updates: Array<{ id: string; data: DrawingUpdateData }>) => {
      const updatedAnnotations =
        await window.electronAPI.drawing.batchUpdate(updates)
      setAnnotations((prev) => {
        const updatedMap = new Map(
          updatedAnnotations.map((ann) => [ann.id, ann]),
        )
...
export function convertAnnotationToLegacy(
  annotation: DrawingAnnotation,
): DrawingElementLegacy {
  return {
    id: annotation.id,
...
export async function exportIndividualResultsPDF(
  ...
  if (options.includeTextboxAnnotations) {
    const textboxes = await getTextboxAnnotationsByQuestionScore(
      questionScore.id,
    )
