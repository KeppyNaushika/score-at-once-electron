...
**未確認のまま進めた点（#1126）**: 同期対象に足す際、**対象テーブルの書き込み方を確認して
いなかった**。`saveAsbDefinition` は定義を delete して作り直すため、保存のたびに全子行の
DELETE + INSERT が changelog と `_tombstone` に流れる。後の実測で、同期と無関係な現行バグも
出ていることが分かった。

| 測定（2026-08-02、開発用DB） | 結果                                                     |
| ---------------------------- | -------------------------------------------------------- |
| FK カスケードが実際に走るか  | Prisma のアダプタ下で `PRAGMA foreign_keys = 1`。走る    |
| タグ紐付けの現存数           | 定義12件に対し `AsbDefinitionTag` は **0件**             |
| `createdAt` のリセット       | `AsbDefinition` **12件すべて** `createdAt` = `updatedAt` |

編集画面を開くだけで保存が走る（`AnswerSheetBuilderMainView.tsx:94-108`）ため、**タグを付けて
編集画面を開くと消える**。`createdAt` は既に全件失われている。

**さらに、この同期対応は現時点では利用者から見えない。** `listAsbDefinitions` が
`where: { userId }` で所有者を絞る（`asbDefinition.ts:81`）ため、**別の教員としてログインすると
同期されてきた定義は一覧に現れない**。一覧が唯一の発見手段である（`getAsbDefinition(id)` 側に
所有者チェックは無いので、id を知っていれば開ける）。

一覧の絞り方は機能ごとに三者三様で、これ自体が設計の未決を表している。
...
したがって #1126 で懸念した「相手端末で定義が消える」も、消える対象がそもそも見えていないので
体感されない。**実害として残るのは同期と無関係な3件**（タグ消失・`createdAt`・開くだけ保存）で、
...
> **作成の衝突はライブラリが解決する**（v0.11.0 がこの用途のために入った）ので、根拠としては
> 成り立っていなかった。2026-08-03 に uuidv4 へ戻し、既存行も
> `20260803110000_unify_ids_to_uuidv4` で振り直した。
>
> ただし決定論的 id には**別の働きがあった**ことを記録しておく。`_tombstone` は
> `(tableName, recordId)` しか持たず、削除は主キーでしか照合できない。id が端末間で一致して
> いれば「削除済みの行を、それより古い挿入が復活させる」のを `isShadowedByTombstone` が
> 止められるが、uuidv4 では止まらない。実測（2端末同期）でも、**2行が一度も出会わないうちに
> 片方が削除されると、その削除は伝わらない**ことを確認している。
>
> それでも uuidv4 を選んだのは、id に同一性を持たせない方針を優先し、この穴を**許容する**と
> 判断したため。窓は「2行がどのDBでも一度も同居しないうちの削除」に限られ、一度収束すれば
> 以後は正常。データも壊れず、失敗した削除自体が2行を出会わせるので**再操作すれば直る**。
> sqlite-nas-sync は他プロジェクトでも使うため、この件でライブラリには手を入れない。
