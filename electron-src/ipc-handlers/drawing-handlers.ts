...
export function setupDrawingHandlers() {
  ...
  ipcMain.handle('drawing:create', async (_, data: DrawingCreateData) => {
    try {
      ...
      return { success: false, error: error instanceof Error ? error.message : '描画アノテーション作成に失敗しました' }
    }
  })
   ...
   */
  ipcMain.handle('drawing:getByQuestionScore', async (_, questionScoreId: string, type?: DrawingType) => {
    try {
      console.log(`🎨 描画アノテーション取得要求: QuestionScore=${questionScoreId}, Type=${type || 'ALL'}`)
      const result = await drawingService.getDrawingAnnotationsByQuestionScore(questionScoreId, type)
      console.log(`✅ 描画アノテーション取得成功: ${result.length}件`)
      return { success: true, data: result }
    } catch (error) {
      console.error('🚫 描画アノテーション取得エラー:', error)
      return { success: false, error: error instanceof Error ? error.message : '描画アノテーション取得に失敗しました' }
    }
  })
  ...
  ipcMain.handle('drawing:update', async (_, id: string, data: DrawingUpdateData) => {
    try {
      ...
      return { success: false, error: error instanceof Error ? error.message : '描画アノテーション更新に失敗しました' }
    }
  })
  ...
  ipcMain.handle('drawing:delete', async (_, id: string) => {
    try {
      ...
      return { success: false, error: error instanceof Error ? error.message : '描画アノテーション削除に失敗しました' }
    }
  })
   ...
   */
  ipcMain.handle('drawing:deleteByQuestionScore', async (_, questionScoreId: string, type?: DrawingType) => {
    try {
      console.log(`🎨 描画アノテーション一括削除要求: QuestionScore=${questionScoreId}, Type=${type || 'ALL'}`)
      await drawingService.deleteDrawingAnnotationsByQuestionScore(questionScoreId, type)
      console.log(`✅ 描画アノテーション一括削除成功`)
      return { success: true }
    } catch (error) {
      console.error('🚫 描画アノテーション一括削除エラー:', error)
      return { success: false, error: error instanceof Error ? error.message : '描画アノテーション一括削除に失敗しました' }
    }
  })
   ...
   * 描画アノテーション一括作成
   */
  ipcMain.handle('drawing:batchCreate', async (_, annotations: DrawingCreateData[]) => {
    try {
      console.log(`🎨 描画アノテーション一括作成要求: ${annotations.length}件`)
      const result = await drawingService.batchCreateDrawingAnnotations(annotations)
      console.log(`✅ 描画アノテーション一括作成成功: ${result.length}件`)
      return { success: true, data: result }
    } catch (error) {
      console.error('🚫 描画アノテーション一括作成エラー:', error)
      return { success: false, error: error instanceof Error ? error.message : '描画アノテーション一括作成に失敗しました' }
    }
  })
   ...
   * 描画アノテーション一括更新
   */
  ipcMain.handle('drawing:batchUpdate', async (_, updates: Array<{ id: string; data: DrawingUpdateData }>) => {
    try {
      ...
      return { success: false, error: error instanceof Error ? error.message : '描画アノテーション一括更新に失敗しました' }
    }
  })
  ...
  ipcMain.handle('drawing:getStats', async (_, questionScoreId: string) => {
    try {
      console.log(`🎨 描画アノテーション統計取得要求: QuestionScore=${questionScoreId}`)
      const result = await drawingService.getDrawingAnnotationStats(questionScoreId)
      console.log(`✅ 描画アノテーション統計取得成功: 合計${result.total}件`)
      return { success: true, data: result }
    } catch (error) {
      console.error('🚫 描画アノテーション統計取得エラー:', error)
      return { success: false, error: error instanceof Error ? error.message : '描画アノテーション統計取得に失敗しました' }
    }
  })
