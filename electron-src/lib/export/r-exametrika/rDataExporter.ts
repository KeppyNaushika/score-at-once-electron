...
export async function exportRData(
  ...
  const exportData = await fetchExportData(examId, selectedExamStudentIds)

  const scoringData = exportData.scoringData
  if (scoringData.length === 0) {
    throw new Error("出力対象の生徒がいません")
  }
  ...
  if (format === "json") {
    content = JSON.stringify(
      {
        exam: { examName: exportData.exam.examName ?? "" },
        questions,
  ...
  if (!finalOutputPath) {
    ...
    const safeExamName = exportData.exam.examName
      ? sanitizeFileName(exportData.exam.examName)
      : null
