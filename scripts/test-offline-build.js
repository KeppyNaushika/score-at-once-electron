  ...
  CRITICAL_OFFLINE_FILES.forEach(file => {
    ...
    if (EXPECTED_SIZES[file]) {
      ...
      if (sizeMB < min || sizeMB > max) {
        console.warn(`⚠️  Size warning: ${file} is ${sizeMB.toFixed(2)}MB (expected ${min}-${max}MB)`)
      }
    }
  })
...
function runDryBuild() {
  ...
  try {
    ...
    console.log(`⚙️  Forge Config: extraResource items = ${forgeConfig.packagerConfig.extraResource.length}`)
    ...
    if (fs.existsSync(nextOutputPath)) {
      ...
      console.warn('⚠️  Next.js build output not found - running npm run build first')
  ...
  const tests = [
    {
      ...
      test: (content) => {
        ...
        const hasMathJaxContent = content.includes('HTML_ENTITIES') || content.includes('MathJax')
    ...
    },
    {
      ...
      test: (content) => content.includes('Mozilla Foundation') && content.length > 500000
    }
  ]
