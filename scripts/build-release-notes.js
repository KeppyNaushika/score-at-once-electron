...
2. 解凍してフォルダ内の実行ファイルを起動
   - macOS: \`score-at-once-mac-*.zip\`をダウンロード・解凍
     **初回起動時**: 「開発元を確認できません」と表示される場合は、\`一括採点.app\`を右クリック→「フォルダに新規ターミナル」を選択し、
     \`xattr -r -d com.apple.quarantine .\` を実行してください。
   - Windows: \`一括採点.exe\`
   - Linux: \`一括採点\``

const license = `### 📄 ライセンス

- 本ソフトウェアは GNU Affero General Public License v3.0（AGPLv3）で提供されています。
...
完全無料・インストール不要・オフライン完結の採点支援ソフトです。
