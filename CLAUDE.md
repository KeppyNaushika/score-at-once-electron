...
- **ソースを読んでも、1つの関数で満足すると同じ誤りになる。** 2026-08-03 の実例: 「別id・同一ユニークキーの行はNAS同期で衝突する」という長年の誤認を正すため `conflict.ts` の `applyInsert`（セカンダリUNIQUE違反をLWWで解決する）を読んで「解決済み」と結論したが、同じファイルの110行上にある `isShadowedByTombstone` を見落とした。**作成の衝突と削除の伝搬は別のキーで動いており**（前者は unique キー、後者は主キー）、半分だけ読んで全体を理解したつもりになっていた
- **挙動の主張は、読むだけでなく走らせて確かめる。** `~/dev/sqlite-nas-sync/__tests__/sync.test.ts` に2端末同期のハーネスがあり、一時テストを足せば数分で実測できる。上記の見落としも、シナリオを1本書けば即座に露見した
