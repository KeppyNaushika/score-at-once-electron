...
import { GradingMode } from "@/components/grading/GradingModeToggle"
...
export function useGradingPage() {
  ...
  const [displayFilter, setDisplayFilter] = useState<{
    ...
    status: ["ungraded", "correct", "incorrect", "partial", "pending", "no_answer"],
    showAll: true,
  })
  ...
  const [scoringData, setScoringData] = useState<{ [key: string]: ScoringData }>({})
  ...
  const loadAnswerSheets = useCallback(async () => {
    try {
      const response = await window.electronAPI.getAnswerSheetsByProjectId(projectId)
      if (response && response.success && response.answerSheets) {
        // 型変換: Prismaの型からAnswerSheet型へ
        const answerSheets = response.answerSheets.map((sheet: {
          id: string
          ...
          imagePath: sheet.originalImagePath || sheet.processedImagePath || '',
