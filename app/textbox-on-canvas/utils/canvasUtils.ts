...
export function setupDebugPreview(
  ...
  try {
    ...
    console.log('🖼️ デバッグプレビュー設定完了:', `${width}x${height}px`)
  } catch (error) {
    console.error('デバッグプレビュー設定エラー:', error)
    ...
    try {
      ...
      img.onload = () => {
        try {
          ...
          console.log('🖼️ Canvas描画スケーリング:', {
            original: { width: originalWidth, height: originalHeight },
            textBox: { width: textBoxWidth, height: textBoxHeight },
            scale: { x: scaleX, y: scaleY, final: scale },
            scaled: { width: scaledWidth, height: scaledHeight },
            offset: { x: offsetX, y: offsetY },
            drawPosition: { x: x + offsetX, y: y + offsetY }
          })
          ...
          // デバッグ用赤枠を描画（測定精度確認用）
          drawDebugBorder(ctx, x + offsetX, y + offsetY, scaledWidth, scaledHeight)
